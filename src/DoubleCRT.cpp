...
void DoubleCRT::FFT(const NTL::ZZX& poly, const IndexSet& s)
{
  ...
  if(my_flag){
    ...
    std::cout << "进入DoubleCRT——FFT" << std::endl;
    my_flag = false;
  }
  ...
  for (long j = first; j < last; j++) {
    // if(my_flag2) {std::cout << "poly:[" << poly << std::endl; my_flag2 = false;}
    long i = ivec[j];
    // if(my_flag2) {std::cout << typeid( context.ithModulus(i) ).name() << std::endl; my_flag2 = false;}
