...
// ### my_edit begin
  ...
  void test_isFFT(NTL::vec_long& y) const;
  // yp = FFT(xp);
  void myFFT(long* yp, long* xp, long dx,long modulus) const;
  // void myFFT(NTL::vec_long& y, NTL::zz_pX& tmp) const;
  void myiFFT(long* xp, const long* yp, long dx,long modulus) const;
  // void myiFFT(long* xp, const long* yp, long twice_root) const;
  void setRoot(long root) { this->root = root; }
  // void set_nttInfo(uint64_t modulus, uint64_t degree){ ntt_info.init(modulus, degree); }
  // Ntt get_nttInfo() { return ntt_info; }
